            ...
            switch (n->type)
            {
                ...
                case GEN_IPADD:
                    ...
                    if (len == 4)
                        ...
                        s = WvString("IP Address: "
                                     "IPv6 - not currently supported");
                    else
                        s = WvString("IP Address: INVALID");
                    break;
                case GEN_OTHERNAME:
                    // We will convert this here to the OID, since it's
                    // easiest to parse
                    OBJ_obj2txt(oneline, sizeof(oneline),
                                n->d.otherName->type_id, 1);

                    if (WvString(oneline) == "1.3.6.1.4.1.311.20.2.3")
                    {
                        ASN1_UTF8STRING *utf8 =
                                n->d.otherName->value->value.utf8string;
                        // Until WvString gets UTF8 friendly, and since
                        // 99% of the time this will be an ASCII
                        // user@kerberose-domain.internal type value, be
                        // careful here and dump this into a WvDynBuf, and
                        // then output the string.
                        buf.put(ASN1_STRING_data(utf8),
                                ASN1_STRING_length(utf8));
                        s = WvString("Othername: Microsoft UPN: %s ",
                                     buf.getstr());
                    }
                    ...
                    /*
                        see
            http://www.ietf.org/mail-archive/web/pkix/current/msg20829.html
                        for ASN.1 structure
                    */
                        s = WvString("Othername: "
                                     "Personal Data: <unsupported> ");
                    }
                    ...
                    else if (WvString(oneline) == "1.3.6.1.5.5.7.8.3")
                    {
                        ...
                        */
                        s = WvString("Othername: "
                                     "Permanent Identifier: <unsupported> ");
                    }
                    else if (WvString(oneline) == "1.3.6.1.5.5.7.8.4")
                    {
                        ...
                        */
                        s = WvString("Othername: "
                                     "Hardware Module Name: <unsupported> ");
                    }
                    else if (WvString(oneline) == "1.3.6.1.5.5.7.8.5")
                    {
                        ...
                        buf.put(ASN1_STRING_data(utf8),
                                ASN1_STRING_length(utf8));
                        s = WvString("Othername: XMPP Address: %s ",
                                     buf.getstr());
                    }
                    else if (WvString(oneline) == "1.3.6.1.5.5.7.8.6")
                    {
                        // see RFC 4683 for ASN.1 structures
                        s = WvString("Othername: SIM: <unsupported> ");
                    }
                    else if (WvString(oneline) == "1.3.6.1.5.5.7.8.7")
                    {
                        ASN1_IA5STRING *ia5 =
                                n->d.otherName->value->value.ia5string;
                        buf.put(ASN1_STRING_data(ia5),
